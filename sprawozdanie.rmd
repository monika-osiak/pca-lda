...
3. Wybraliśmy niestety zbiór, który okazał się trudny do liniowej eliminacji wymiarów i dla żadnej z metod nie otrzymaliśmy bardzo dobrych wyników.Jednak dla naszego modelu LDA lepiej sklasyfikowało dane.
...
5. LDA wymaga wcześniejszego przeprocesowania danych, przez co jest trudniejsze w użyciu. Równocześnie ze względu na jednowymiarowość wyników jest metodą trudną w prezentacji dla niewielkiej ilości klas.

6. Przypuszczamy, że w większości sytuacji metoda LDA powinna dawać lepsze wyniki, jednakże wszystko zależy od omawianego przypadku i do każdego należy podejść indywidualnie.